...
export function DataTableRowActions<TData>({
  ...
  const mutation = trpc.label.remove.useMutation()
  const { user } = useUser()
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. This will permanently delete {row.getValue("key")} from the system.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel
            disabled={mutation.isLoading || labels.isLoading || labels.isRefetching || labels.isFetching}
          >Cancel</AlertDialogCancel>
          <Button variant="destructive"

            onClick={() =>
              mutation.mutate((row.original as Label), {
                onSuccess: () => {
                  utils.label.getAll.refetch().then(() => {
                    setOpen(false)
                    toast.success(dict?.labelDeletedSuccessfully || "Label deleted successfully")
                  })
                }
              })
            }
